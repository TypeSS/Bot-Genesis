  ...
  async execute(interaction) {
    ...
    if (all) {
      ...
      await interaction.editReply({
        content: `${members.size} membros. A preparar para sincronizar...`,
      });
...
function syncLevel(interaction: ChatInputCommandInteraction, member: GuildMember, single = true) {
  const level = levelDb.getLevelByRoles(
    interaction.guildId!,
  ...
  if (level) {
    ...
    if (single)
      interaction.reply({
        content: `Nível ${level} atribuído a ${member.displayName}.`,
        flags: MessageFlags.Ephemeral,
      });
  } else if (single) {
    interaction.reply({
      content: `${member.displayName} não tem nenhuma role de nível.`,
      flags: MessageFlags.Ephemeral,
    });
  }
}
