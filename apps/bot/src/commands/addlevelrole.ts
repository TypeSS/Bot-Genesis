...
export const addLevelRole: SlashCommand = {
  data: new SlashCommandBuilder()
    ...
    .addNumberOption((input) =>
      input.setName("level").setDescription("Level que terá a role").setRequired(true),
    )
    .addRoleOption((input) => input.setName("role").setDescription("Role a dar").setRequired(true)),
  async execute(interaction) {
    ...
    interaction.reply({
      content: `Role \`${role.name}\` adicionada ao nível ${level}.`,
